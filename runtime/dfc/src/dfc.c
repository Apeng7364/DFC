...
{ //地址，地址，地址
  ...
  if (F->stop == 0)
  {
    ...
    for (int i = 0; i < Num; i++)
    {
      ...
      if (b->MaxSize == b->Data_Count)
      { //扩容
        // printf("%s扩容\n",F->Name );
        void *p = (void *)malloc(b->MaxSize * b->persize * 2);

        memcpy(p, (char *)b->Data + b->Head * b->persize, (b->MaxSize - b->Head) * b->persize);
        memcpy((char *)p + (b->MaxSize - b->Head) * b->persize, b->Data, b->Head * b->persize);
...
{ //void 修改为返回当前是A函数的第几次运行
  ...
  for (int i = 0; i < data_count; i++)
  {
    ...
    memcpy(data_addr, (char *)temp_ad->Data + ((finish_num - temp_ad->Order + temp_ad->Head) % temp_ad->MaxSize) * temp_ad->persize, data_size); // copy data

    // should make sure that temp_ad's data only operated by one thread

    // update list start
    memcpy(&rest_count, (char *)temp_ad->Data + ((finish_num - temp_ad->Order + temp_ad->Head) % temp_ad->MaxSize) * temp_ad->persize + data_size, INTSIZE); //修改点： 改为赋值
    ...
    if (rest_count > 1)
    {
      ...
      memcpy((char *)temp_ad->Data + ((finish_num - temp_ad->Order + temp_ad->Head) % temp_ad->MaxSize) * temp_ad->persize + data_size, &rest_count, INTSIZE);
    }
    else
    {
      // rest is one, clean
...
// int *count_list = (int*)malloc(sizeof(int) * table->Num);
#ifdef DEBUG
  int begin, end;
  begin = time(NULL);
#endif
...
void DF_Update_output(DF_TFL *table)
{
  ...
  for (int i = 0; i < table->output_list_len; i++)
  {
    ...
    {
      memcpy((char *)table->result[i] + (j * data_size),
             (char *)temp_ad->Data + ((temp_ad->Head + j) * temp_ad->persize), data_size);
    }
  }
}
